#!/usr/bin/env python3
# Crea/actualiza items TRAPPER para network_rejection
...
WOLKVOX_URL    = "https://wv0025.wolkvox.com/api/v2/real_time.php"
WOLKVOX_SERVER = "00XX"
WOLKVOX_TOKEN  = "TOKEN"
...
def main():
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] === NETWORK REJECTION ITEMS SYNC ===")
    ...
    for idx, (code, name) in enumerate(sorted(agents.items()), 1):
        key_ = f"redplus.agent.nr[{code}]"
        item_name = f"redplus.Agent-{code}-{name}-NR"
        it = item_by_key(auth, hostid, key_)
        try:
            if it:
                api("item.update", {
                    "itemid": it["itemid"], "name": item_name,
                    "type": 2, "value_type": 3, "units": "%",
                ...
                api("item.create", {
                    ...
                    "type": 2, "value_type": 3, "units": "%",
                    "history": "90d", "trends": "365d",
                    "description": f"Network rejection del agente {code}"
                }, auth)
                created += 1
