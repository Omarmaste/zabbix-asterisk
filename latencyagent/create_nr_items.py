#!/usr/bin/env python3
# Crea/actualiza items TRAPPER para network rejection de agentes
# Un item por agente: Continente.agent.nr[agent_code]
...
def main():
    ...
    for idx, (code, name) in enumerate(sorted(agents.items()), 1):
        key_ = f"stargroup.agent.nr[{code}]"
        item_name = f"stargroup.Agent-{code}-{name}-NR"
